...
DEFAULT_EXECUTION_CONFIG: dict[str, Any] = {
    ...
    "rss_quote_file": "execution/rss_realtime_quotes.csv",
    "rss_quote_max_age_seconds": 15,
    "require_bid_ask_for_live": True,
    "require_trading_status_for_live": True,
    ...
    "analysis_reuse_today_cache": True,
    "analysis_force_refresh": False,
    "yahoo_max_attempts": 2,
    "yahoo_cooldown_minutes": 60,
    "yahoo_chunk_size": 10,
    "yahoo_chunk_pause_seconds": 3,
    "yahoo_threads": False,
}
...
DEFAULT_TEAM_CONFIG: dict[str, Any] = {
    ...
    "members": [
        {"role": "チーフトレーダー", "responsibility": "最終売買許可・執行ルール監督"},
        {"role": "プロトレーダー", "responsibility": "売買戦略・相場判断"},
        {"role": "リスクマネージャー", "responsibility": "損失上限・資金配分・緊急停止"},
        {"role": "クオンツ", "responsibility": "期待値・バックテスト・最適化"},
        {"role": "データエンジニア", "responsibility": "Yahoo・RSS・CSV・キャッシュ品質管理"},
        {"role": "プロコーダー", "responsibility": "実装品質・保守性・互換性"},
        {"role": "デバッカー", "responsibility": "構文・入出力・設定・異常系検査"},
        {"role": "QAテスター", "responsibility": "自動テスト・回帰テスト・安全性検証"},
        {"role": "システム監査役", "responsibility": "LIVE移行条件・証跡監査"},
    ],
}
...
def bootstrap_environment() -> dict[str, Any]:
    for directory in (CONFIG_DIR, REPORT_DIR, EXECUTION_DIR, STATE_DIR, LOG_DIR, DATA_DIR, CACHE_DIR):
...
def validate_execution_config(config: dict[str, Any]) -> list[str]:
    ...
    if safe_float(config.get("rss_quote_max_age_seconds"), 0) <= 0:
        errors.append("rss_quote_max_age_seconds は0より大きい必要があります")
    spread = safe_float(config.get("maximum_spread_percent"), -1)
    if not 0 <= spread <= 10:
        errors.append("maximum_spread_percent は0～10である必要があります")
