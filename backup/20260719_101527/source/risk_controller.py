...
from execution_core import (
    ROOT_DIR, REPORT_DIR, STATE_DIR, bootstrap_environment, configure_console,
...
def main() -> None:
    ...
    adopted = (
        position_plan[position_plan["Position判定"].astype(str).eq("採用")].copy()
        if not position_plan.empty and "Position判定" in position_plan.columns else pd.DataFrame()
    )
    ...
    add("総エクスポージャー", exposure <= safe_float(config.get("maximum_total_exposure_percent", 30.0), 30.0), f"予定 {exposure:.2f}% / 上限 {safe_float(config.get('maximum_total_exposure_percent', 30.0)):.2f}%")
    ...
    summary = {
        "version": "PHOENIX v6.5.1", "generated_at": now_text(), "mode": mode,
        ...
        "config_auto_created": bool(boot["config_created"]), "checks": checks,
    }
    save_json(RISK_SUMMARY_FILE, summary)
    lines = ["PHOENIX v6.5.1 RISK CONTROLLER", "=" * 100, f"生成時刻: {summary['generated_at']}", f"モード: {mode}", f"判定: {summary['decision']}", f"設定自動生成: {summary['config_auto_created']}", ""]
    lines.extend(f"{'OK' if i['ok'] else 'BLOCK':<6} {i['name']}: {i['detail']}" for i in checks)
    RISK_REPORT_FILE.write_text("\n".join(lines) + "\n", encoding="utf-8")

    print("=" * 100); print("PHOENIX v6.5.1 RISK CONTROLLER"); print("=" * 100)
    print(f"モード : {mode}"); print(f"判定   : {summary['decision']}")
    if boot["config_created"]: print("設定   : config/execution_config.json を安全な初期値で自動生成")
